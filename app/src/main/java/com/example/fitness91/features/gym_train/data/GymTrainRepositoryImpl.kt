package com.example.fitness91.features.gym_train.data

import com.example.fitness91.R
import com.example.fitness91.core.data.utils.GymExeRecordTableColumns
import com.example.fitness91.core.data.utils.GymExeTableColumns
...
import com.example.fitness91.features.gym_train.data.mapper.toExercise
import com.example.fitness91.features.gym_train.data.mapper.toTodayData
import com.example.fitness91.features.gym_train.data.mapper.toUpdateCompleted
...
class GymTrainRepositoryImpl @Inject constructor(
    ...
    override suspend fun getTodayExercise(): Result<List<Exercise>> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val exerciseList = supDatabase[Tables.GYM_EXERCISE].select {
                    filter {
                        eq(GymExeTableColumns.DAY, todayDay)
                    }
                    order(GymExeTableColumns.ID, Order.ASCENDING)
        ...
        return withContext(Dispatchers.IO) {
            try {
                ...
                sessionManager.waitForSession()?.id?.let {
                    val todayDataList = supDatabase[Tables.GYM_EXERCISE_RECORD].select(
                        columns = Columns.list(
                            GymExeRecordTableColumns.EXERCISE_ID,
                            GymExeRecordTableColumns.SETS
                        )
                    ) {
                        filter {
                            eq(GymExeRecordTableColumns.CREATED_BY, it)
                            eq(GymExeRecordTableColumns.CREATED_AT, todayDate)
                        }
