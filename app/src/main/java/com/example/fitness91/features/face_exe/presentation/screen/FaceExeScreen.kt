...
import com.example.fitness91.features.face_exe.presentation.components.ExerciseItemUI
...
fun FaceExeScreen(
    ...
    val exerciseState by faceExeViewmodel.exerciseListState.collectAsStateWithLifecycle()
    val isRefresh by faceExeViewmodel.isRefresh
    ...
    Column(
        ...
        HeaderBackIcon(headerText = stringResource(R.string.face_exercise), onBackClick = onBackClick)

        PullToRefreshBox(
            isRefreshing = isRefresh,
            onRefresh = { faceExeViewmodel.getTodayExercise() },
        ) {
            LazyColumn(
                modifier = Modifier
                    .padding(top = 8.dp)
                    ...
                    is AppState.Success -> {
                        ...
                        items(items = exercises, key = { it.id }) { exercise ->
                            ExerciseItemUI(
                                exercise = exercise,
                                onExerciseClick = {
                                    faceExeViewmodel.setToBeCompletedExe(exercise)
                                    onExerciseClick()
                                }
                            )
                        }
