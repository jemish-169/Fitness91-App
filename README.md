# Fitness91

## Overview  
**Fitness91** is a fitness tracking Android application designed get daily workout routines, facial exercises, and diet plans. Built using Jetpack Compose and powered by Supabase, it provides real-time progress tracking, dynamic dashboards, and plans based on the current weekday.

## Features

- **User Authentication**  
  - Secure and efficient login/signup using Supabase Auth.  

- **Daily Fitness Modules**  
  - **Gym Training**: Tailored daily workout plans.  
  - **Face Exercise**: Simple facial workouts to relax and tone.  
  - **Diet Planning**: Suggested meals and hydration tracking.  
  - Daily plans are fetched from Supabase based on the current weekday.  

- **Progress Tracking & Visualization**  
  - Track completed exercises and diet goals.  
  - Visual feedback with color-coded progress indicators.  
  - View historical performance on a daily and monthly basis.

- **Dynamic Dashboard**  
  - Highlights today’s plan and current progress.  
  - Module-wise and overall progress displayed clearly.  
  - Adaptive UI with support for dark, light, and system themes.

## Technology Stack

- **Android (Kotlin)** – Native Android development using Jetpack Compose  
- **Supabase** – Backend-as-a-service for authentication, real-time data sync, and database  
- **Jetpack Compose** – Declarative UI toolkit for clean, efficient layouts  
- **Coroutines** – Managing background tasks smoothly  
- **Hilt** – Dependency injection for modular, testable architecture  
- **MVVM + Clean Architecture** – Ensures a scalable, maintainable, and test-friendly codebase  
- **Material Design** – Follows Google’s UI/UX guidelines for intuitive design  
- **Animations** – Enhances user experience with meaningful transitions and effects

## Screenshots  
| Sign up Screen | SignIn screen | Home Screen |
|-------------|----------------|--------------------------|
| ![0](https://github.com/user-attachments/assets/97ec7774-23a2-44cd-95a7-3f0d5efd42e6) | ![1](https://github.com/user-attachments/assets/40f8dd1a-bdb7-4b2e-a473-d73ed1f0ad27) | ![2](https://github.com/user-attachments/assets/9ca2efe8-5596-421f-89d6-c278d6dcf480) |

| Gym Training tasks | Complete Exercises | Face Exercises |
|-------------|----------------|--------------------------|
| ![3](https://github.com/user-attachments/assets/8228ba0c-d9a5-4534-860a-db5160359f09) | ![4](https://github.com/user-attachments/assets/c4fe119f-1bf4-4932-88b6-648070aa96ba) | ![5](https://github.com/user-attachments/assets/a65d61cc-78df-4d89-ab90-f5418569413f) |

| Diet Plan | Daily insights | App settings |
|-------------|----------------|--------------------------|
| ![6](https://github.com/user-attachments/assets/912f0027-a426-4923-83fb-5276410c989f) | ![7](https://github.com/user-attachments/assets/b5e806f9-864a-4e94-adac-d50ef70f30d1) | ![8](https://github.com/user-attachments/assets/6b48b6f7-7527-4368-b3ed-b3975a4330c0) |
